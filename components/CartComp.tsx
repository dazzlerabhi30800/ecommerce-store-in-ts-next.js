        ...
        <div className="cart--item relative flex text-left items-center gap-4 p-2 bg-pink-300 rounded-md shadow-md">
            ...
            <div className="flex flex-col gap-4">
                ...
                <div className="flex bg-gray-300 rounded-lg items-center text-black text-sm md:text-base w-fit mx-initial font-medium shadow-xl">
                    <button onClick={() => minusToCart(cart.id)} className="py-1 md:py-2 px-2 md:px-4 border-r-2 border-white">-</button>
                    <span className="py-1 px-2 md:py-2 md:px-4 border-r-2 border-white">{cart.quantity}</span>
                    <button onClick={() => addToCart(cart.id)} className="py-1 px-2 md:py-2 md:px-4">+</button>
                </div>
                <div className="font-bold">
                    ${INTEGER_FORMATTER.format((handleDiscount(cart.price, cart.discount)) * cart.quantity)}
                </div>
            </div>
            <button onClick={() => removeItem(cart.id)} className="removeBtn text-2xl text-gray-800 absolute bottom-3 right-2 md:top-5 md:bottom-auto md:right-5 transition ease-in-out duration-300 hover:text-gray-600 md:cursor-pointer">
                <BsFillTrashFill />
            </button>
        </div>
    )
